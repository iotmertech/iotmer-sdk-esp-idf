...
Small **[Bleak](https://github.com/hbldh/bleak)** script for **IOTMER BLE Wi‑Fi provisioning (GATT v1)** and optional **Claim** JSON on UUID `…0404…` (`claim.set` / `claim.ack` on **Events**). Use against firmware built from the **05** example ([monorepo](https://github.com/iotmertech/iotmer-sdk-esp-idf/tree/main/examples/05_ble_wifi_prov) or the copy under `components/iotmer/examples/05_ble_wifi_prov` in the `iotmer` package).

Protocol: [ble-wifi-provisioning.md](https://github.com/iotmertech/iotmer-sdk-esp-idf/blob/main/docs/sdk/esp-idf/ble-wifi-provisioning.md).

Firmware on the board must be built with the **mbedTLS certificate bundle** options in this example’s **`sdkconfig.defaults`** (full + cross-signed) so **HTTPS provision** and **bind-claim** after BLE succeed; see the **HTTPS / TLS** section in that doc.
...
```bash
cd /path/to/this/folder   # e.g. …/examples/05_ble_wifi_prov/pc_ble_client or …/05_ble_wifi_prov/pc_ble_client
python3 -m venv .venv
